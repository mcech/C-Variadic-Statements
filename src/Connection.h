...
public:
     ...
     * @throws SQLException  If a database access error occurs
     */
    template <class... Params> std::unique_ptr<Result> executeQuery(const std::string& sql, Params&&... params);
...
inline std::unique_ptr<Result> Connection::executeQuery(const std::string& sql, Params&&... params)
{
    SQLHSTMT stmt = {};
    ...
    return std::unique_ptr<Result>(new Result(stmt));
}
